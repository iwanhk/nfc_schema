...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python genExcel.py <NAME> <ARG> <NUMBER>")
        sys.exit(1)
    ...
    name= sys.argv[1]
    arg= sys.argv[2]
    totalNumber= int(sys.argv[3])
    excelFile = name+ sys.argv[3]+ ".xlsx"
    ...
    for sn in tqdm(range(last_max, last_max+totalNumber ), desc="生成数据", unit="个"):
        ...
        schema = generator.generate_nfc_scheme(arg, sn, code, env_version)  # 可以根据需要修改为 "trial" 或 "develop"
        if schema:
            # 更新 .env 文件中的 LAST_MAX_COLUMNS
            set_key(".env", "LAST_MAX_COLUMNS", str(sn+1))
            SN.append(sn)
            SCHEMA.append(schema)
            QUERY.append(f"{arg}{code}")
            CODE.append(code)
